...
def test_full_flow(api_client: TestClient, auth_headers: dict[str, str]) -> None:
    ...
    resp = api_client.get("/v1/portfolios/u1/p1/holdings", headers=auth_headers)
    ...
    resp = api_client.get("/v1/portfolios/u1/p1/valuation", headers=auth_headers)
    assert resp.status_code == 200
    val = resp.json()
...
def test_trades_pagination(api_client: TestClient, auth_headers: dict[str, str]) -> None:
    ...
    # First page of 2.
    resp = api_client.get("/v1/portfolios/u1/p/trades", params={"limit": 2}, headers=auth_headers)
    assert resp.status_code == 200
    page1 = resp.json()
...
def test_get_portfolio_404(api_client: TestClient, auth_headers: dict[str, str]) -> None:
    resp = api_client.get("/v1/portfolios/u1/nope", headers=auth_headers)
...
def test_price_unknown_symbol_404(api_client: TestClient, auth_headers: dict[str, str]) -> None:
    resp = api_client.get("/v1/prices/dogecoin", headers=auth_headers)
    assert resp.status_code == 404
...
def test_price_known_symbol(api_client: TestClient, auth_headers: dict[str, str]) -> None:
    resp = api_client.get("/v1/prices/ethereum", headers=auth_headers)
    assert resp.status_code == 200
    body = resp.json()
...
def test_alerts_crud(api_client: TestClient, auth_headers: dict[str, str]) -> None:
    ...
    # List.
    resp = api_client.get("/v1/portfolios/u1/p/alerts", headers=auth_headers)
    assert resp.status_code == 200
    assert [a["alert_id"] for a in resp.json()] == [alert_id]

    # Get.
    resp = api_client.get(f"/v1/portfolios/u1/p/alerts/{alert_id}", headers=auth_headers)
    assert resp.status_code == 200
    assert resp.json()["alert_id"] == alert_id

    # Delete -> 204.
    resp = api_client.delete(f"/v1/portfolios/u1/p/alerts/{alert_id}", headers=auth_headers)
